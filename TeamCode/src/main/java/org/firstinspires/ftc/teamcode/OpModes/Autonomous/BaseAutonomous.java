...
public abstract class BaseAutonomous extends LinearOpMode {
    ...
    public void driveToJunctionHigh() {
        ...
        //Drive to x = 0
        navi.drive_to_pos(-10.0*fx,161.0*fz,0.3,0.3);
        while (navi.drive && opModeIsActive()) {
            navi.step();
            output();
        }
        ...
        //Drive against wall
        navi.drive_to_pos(-10.0*fx,182.0*fz,0.4,1);
        while (navi.drive && opModeIsActive()) {
            navi.step();
            output();
        }
        ...
        navi.drive_to_pos(0,77.0*fz,0.2,0.3);
        while (navi.drive && opModeIsActive()) {
            navi.step();
            output();
        }
    ...
    public void driveToZone() {
        ...
        // drive forwards
        navi.drive_to_pos(navi.position_x, (30)*fz, 0.4, 0.5);
        while (navi.drive && opModeIsActive()) {
            navi.step();
            output();
        }
        ...
        //drive to specific zone
        navi.drive_to_pos(navi.position_x + dx*fx, navi.position_z, 0.3, 0.5);
        while (navi.drive && opModeIsActive()) {
            navi.step();
        }
    }
    ...
    public void detectSignal() {
        ...
        robot.motor_lift.setTargetPosition((int) lift_start_encoder_value - 3600);
